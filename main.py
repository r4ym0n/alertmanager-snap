...
def get_id_from_alerts(alerts):
    ...
    if info['type'] == 1:
        instanceIds = list(
            map(lambda x: x['labels']["instance"], alerts["alerts"]))
        instanceNms = list(
            map(lambda x: x['labels']["instance_name"], alerts["alerts"]))
    elif info['type'] == 2:
        instanceIds = list(
            map(lambda x: x['labels']["instance"], alerts["alerts"]))
        instanceNms = list(
            map(lambda x: x['labels']["instanceName"], alerts["alerts"]))
    # 使用两个纬度来定位一个资源
...
def get_time_range(timestr: str):
    GRAPH_DURATION = 50

    start_time = int(datetime.strptime(
        timestr, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp()) - (GRAPH_DURATION*60)
    end_time = int(datetime.strptime(
        timestr, '%Y-%m-%dT%H:%M:%S.%fZ').timestamp())
    return start_time, end_time


def extract_query_expr(query):
    query_expr = re.search(r"g0\.expr=(.*?)&", query).group(1)
    return query_expr


def extract_alert_info(alert):
    generator_url = alert["alerts"][0]["generatorURL"]
    status = alert["alerts"][0]["status"]
    fingerprint = alert["alerts"][0]["fingerprint"]
    ext_url = alert["alerts"][0]['labels']["ext_url"]
    query_expr = re.search(r"g0\.expr=(.*?)&", generator_url).group(1)
    print(query_expr)
    # replace comparison strings, quary all serial...
    query_expr = regenerate_quary_for_instance(alert)
    print(query_expr)
    return {
        "status": status,
...
def handle_alert_svg():
    ...
    alert_body = extract_alert_info(alerts_data)

    if alerts_data["alerts"][0]["status"] == "reslved":
        start_time, end_time = get_time_range(
            alerts_data["alerts"][0]["endsAt"])
    else:
        start_time, end_time = get_time_range(
            alerts_data["alerts"][0]["startsAt"])

    # 提取告警中的特征ID
    graph_data = get_graph_data_raw(
        prometheus_url=prometheus_url,
        query=alert_body["query_expr"],
        start_time=start_time,
        end_time=end_time,
    ...
    if graph_data == None:
        logging.warning(
            "graph_data is None, query_expr: {}".format(alert_body["query_expr"]))
        # status = 404
        abort(404)

    graph_data_of_instances = data_instance_filter(
        alerts_data, graph_data, get_id_from_alerts(alerts_data))
    ...
    serial_data = make_serial_data(
        graph_data_of_instances, alert_body["query_expr"])

    svg_name = plot_multi_line_svg(
        serial_data['title'], serial_data['serials'])
    target_name = f"{alert_body['fingerprint']}-{alert_body['status']}.png"
